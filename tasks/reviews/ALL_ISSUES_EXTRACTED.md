...
|------|----|-----------|-------------|---------|
| [x] | 59_CRIT_01 | `src/app/dashboard/account/page.tsx:22-43` | Account page has no try/catch on Promise.all — any query failure crashes the page with unhandled rejection | 59_QA_TESTER (BUG-026) |
| [x] | 59_CRIT_02 | `src/components/dashboard/monitoring-dashboard.tsx:166` | CPU icon color logic has wrong precedence — `> 75` checked before `> 90`, so red branch is unreachable; CPU at 95% shows yellow instead of red | 59_QA_TESTER (BUG-027) |
| [x] | 59_CRIT_03 | `src/app/api/vps/password/route.ts:113` | Dashboard password stored in plaintext in Supabase — if DB is compromised, all customer passwords exposed | 59_BACKEND_DEV (C1), 59_SECURITY_AUDITOR (C1) |
| [x] | 59_CRIT_04 | All VPS routes | SSH credentials (root passwords) stored in plaintext in database — DB breach exposes root SSH access to every VPS | 59_SECURITY_AUDITOR (C2) |
| [x] | 59_CRIT_05 | `src/app/api/payments/verify/route.ts:17` | Payment verification route missing request.json() try/catch — malformed body returns 500 with potential stack trace leak | 59_BACKEND_DEV (C2) |
| [x] | 59_CRIT_06 | `src/app/api/payments/verify/route.ts` | Payment verification has no rate limiting — attacker can brute-force payment signatures | 59_BACKEND_DEV (C3) |

### HIGH

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | 59_HIGH_01 | `src/app/dashboard/page.tsx:114` | No-subscription "View Plans" link points to /billing which shows "No Active Subscription" with no way to subscribe — user stuck in loop | 59_QA_TESTER (BUG-001) |
| [x] | 59_HIGH_02 | `src/app/dashboard/models/page.tsx:13` | Models page returns null for unauthenticated users instead of redirecting to /login — shows blank page | 59_QA_TESTER (BUG-008) |
| [x] | 59_HIGH_03 | `src/components/dashboard/agent-manager.tsx:205-274` | Bulk deploy does not check VPS status — fires N sequential error toasts when VPS is offline | 59_QA_TESTER (BUG-011) |
| [x] | 59_HIGH_04 | `src/components/dashboard/agent-chat.tsx:410` | Chat sidebar not responsive on mobile — fixed 256px width leaves no room for chat on small screens; chat-mobile-sidebar.tsx exists but is not used | 59_QA_TESTER (BUG-016), 59_UX_REVIEWER |
| [x] | 59_HIGH_05 | `src/components/dashboard/agent-chat.tsx:134-235` | Streaming chat does not abort on agent switch — old stream continues writing to state, accumulating tokens in memory for a non-visible message | 59_QA_TESTER (BUG-017) |
| [x] | 59_HIGH_06 | `src/components/dashboard/channel-manager.tsx:296-331` | Channel reconnect sends empty credentials — always fails for Telegram/Discord/Slack/Teams (only webchat works) | 59_QA_TESTER (BUG-020) |
| [x] | 59_HIGH_07 | `src/lib/rate-limit.ts` | In-memory rate limiter resets on serverless cold starts — rate limits are effectively useless on Vercel | 59_BACKEND_DEV (H1), 59_SECURITY_AUDITOR (M2) |
| [x] | 59_HIGH_08 | `src/app/api/vps/ssl-check/route.ts:23` | SSL check uses regular Supabase client instead of admin client — may silently return null due to RLS | 59_BACKEND_DEV (H2) |
| [x] | 59_HIGH_09 | `src/app/api/webhooks/route.ts:82` | webhooks POST missing try/catch around request.json() | 59_BACKEND_DEV (H3) |
| [x] | 59_HIGH_10 | `src/app/api/keys/[id]/route.ts:92` | keys PATCH missing try/catch around request.json() | 59_BACKEND_DEV (H4) |
| [x] | 59_HIGH_11 | `src/app/api/payments/create-order/route.ts:23` | payments create-order missing try/catch around request.json() | 59_BACKEND_DEV (H5) |
| [x] | 59_HIGH_12 | `src/app/api/cron/mc-recurring/route.ts` and `src/app/api/cron/webhook-retry/route.ts` | Two cron endpoints have ZERO authentication — any unauthenticated request triggers admin operations | 59_SECURITY_AUDITOR (H1) |
| [x] | 59_HIGH_13 | `src/app/api/cron/log-alerts/route.ts:19` | log-alerts cron has weak auth — if CRON_SECRET is unset, check is bypassed entirely (fail-open) | 59_SECURITY_AUDITOR (H2) |
| [x] | 59_HIGH_14 | `src/middleware.ts:152-182` | Middleware does not protect API routes — no /api/* in matcher; any route missing its auth check is fully public | 59_SECURITY_AUDITOR (H3) |
| [x] | 59_HIGH_15 | `src/middleware.ts:75` | Middleware uses getSession() instead of getUser() — expired/revoked sessions can still access dashboard pages | 59_SECURITY_AUDITOR (H4) |
| [x] | 59_HIGH_16 | Multiple V1 routes, webhook-retry cron | SSRF risk — isPrivateUrl() may be bypassable via DNS rebinding, IPv6 loopback, or octal notation | 59_SECURITY_AUDITOR (H5) |
| [x] | 59_HIGH_17 | Multiple server pages | Excessive `any` types — ~40 `any` types across server pages for Supabase query results | 59_FRONTEND_DEV |
| [x] | 59_HIGH_18 | Multiple components | Missing ARIA labels on icon-only buttons — 15+ instances across 7 components | 59_FRONTEND_DEV |
| [x] | 59_HIGH_19 | `src/components/dashboard/vps-maintenance.tsx:268` | Toggle button is 2px dot (h-2 w-2) — far below 44px WCAG minimum touch target | 59_FRONTEND_DEV |
| [x] | 59_HIGH_20 | `logs-explorer.tsx`, `knowledge-base-manager.tsx` | Missing debounce on search inputs — re-renders full table on every keystroke | 59_FRONTEND_DEV |

### MEDIUM

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | 59_MED_01 | `src/app/dashboard/page.tsx:82-86` | Model query error silently ignored — modelRes.error never checked | 59_QA_TESTER (BUG-003) |
| [x] | 59_MED_02 | `src/app/dashboard/vps/page.tsx:46` | VPS page swallows Supabase errors — treats RLS failure as "no VPS" | 59_QA_TESTER (BUG-004) |
| [x] | 59_MED_03 | `src/components/dashboard/vps-controls.tsx:270` | VPS Controls useEffect dependency on [monitoring] may cause chart data stalling | 59_QA_TESTER (BUG-005) |
| [x] | 59_MED_04 | `src/components/dashboard/vps-controls.tsx:362-439` | Double-click timing window on Stop/Restart AlertDialog — actionLoading not set before performAction | 59_QA_TESTER (BUG-007) |
| [x] | 59_MED_05 | `src/app/dashboard/models/page.tsx:39` | availableModels error not checked — empty model list shown on query failure | 59_QA_TESTER (BUG-009) |
| [x] | 59_MED_06 | `src/components/dashboard/agent-manager.tsx:374` | Agent name access on potentially null agents relation — runtime crash possible | 59_QA_TESTER (BUG-012) |
| [x] | 59_MED_07 | `src/components/dashboard/agent-manager.tsx:126` | Deploy limit check uses stale deployedCount during bulk deploy | 59_QA_TESTER (BUG-013) |
| [x] | 59_MED_08 | `src/app/dashboard/store/[id]/page.tsx:179` | Price display shows cost ($X.XX) — may violate project naming rules ("no cost shown") | 59_QA_TESTER (BUG-015) |
| [x] | 59_MED_09 | `src/components/dashboard/channel-manager.tsx:252-277` | fetchLastMessages runs on every render due to channels dependency — N API calls on every state change | 59_QA_TESTER (BUG-019) |
| [x] | 59_MED_10 | `src/app/dashboard/support/new/page.tsx:78,93,179` | New ticket navigation uses /dashboard/support prefix instead of clean /support URLs | 59_QA_TESTER (BUG-021) |
| [x] | 59_MED_11 | `src/components/dashboard/ticket-thread.tsx:203` | Ticket thread "Back to Support" uses /dashboard/support prefix | 59_QA_TESTER (BUG-022) |
| [x] | 59_MED_12 | `src/components/dashboard/ticket-list.tsx:166,216,237` | Ticket list navigation uses /dashboard/support/... paths | 59_QA_TESTER (BUG-023) |
| [x] | 59_MED_13 | `src/app/api/vps/password/route.ts:70` | No max-length on VPS dashboard password — multi-MB string could be written to DB | 59_BACKEND_DEV (M1) |
| [x] | 59_MED_14 | `src/app/api/tickets/create/route.ts` | No max-length on ticket subject/description | 59_BACKEND_DEV (M2) |
| [x] | 59_MED_15 | `src/app/api/tickets/[id]/reply/route.ts` | No max-length on ticket reply message | 59_BACKEND_DEV (M3) |
| [x] | 59_MED_16 | `src/app/api/chat/send/route.ts:49` | No max-length on chat message — could cause large token consumption | 59_BACKEND_DEV (M4) |
| [x] | 59_MED_17 | `src/app/api/webhooks/route.ts` | No max-length on webhook description | 59_BACKEND_DEV (M5) |
| [x] | 59_MED_18 | `src/app/api/agents/config/route.ts` | No max-length on agent config values | 59_BACKEND_DEV (M6) |
| [x] | 59_MED_19 | `src/app/api/notifications/route.ts:55` | Notification PATCH body.ids array not length-limited — could send thousands of IDs | 59_BACKEND_DEV (M7) |
| [x] | 59_MED_20 | `src/app/api/audit-log/route.ts:52-55` | Audit log search sanitization is fragile — strips some but not all special characters | 59_BACKEND_DEV (M8) |
| [x] | 59_MED_21 | `src/app/api/vps/status/route.ts:71` | VPS status leaks internal error message in _stale_reason field | 59_BACKEND_DEV (M9), 59_SECURITY_AUDITOR (L2) |
| [x] | 59_MED_22 | `src/app/api/chat/send/route.ts` | Chat send route has 60s timeout but no maxDuration export — Vercel Hobby tier kills at 10s | 59_BACKEND_DEV (M10) |
| [x] | 59_MED_23 | `src/app/api/account/delete/route.ts` | Account deletion misses 15+ tables — zombie SSH credentials, PII persists, GDPR violation risk | 59_SECURITY_AUDITOR (M1) |
| [x] | 59_MED_24 | `src/app/api/vps/logs/route.ts:46` | VPS logs lines parameter NaN handling is sloppy — parseInt of non-numeric returns NaN | 59_SECURITY_AUDITOR (M3) |
| [x] | 59_MED_25 | `src/app/api/tickets/attachment/route.ts:101-102` | Ticket attachments use public URLs — accessible without authentication to anyone who knows the URL | 59_SECURITY_AUDITOR (M4) |
| [x] | 59_MED_26 | `src/app/api/account/avatar/route.ts:56` | Avatar upload validates MIME type from client-provided Content-Type only — SVG with XSS or spoofed type possible | 59_SECURITY_AUDITOR (M5) |
| [x] | 59_MED_27 | `src/app/api/payments/verify/route.ts:50-51,73-119` | Payment verification trusts client-provided metadata for plan fulfillment — user could pay $1 but claim Pro plan | 59_SECURITY_AUDITOR (M7) |
| [x] | 59_MED_28 | `src/components/dashboard/vps-controls.tsx` (~825 lines) | VPS controls component too large — should be split into 4 sub-components | 59_FRONTEND_DEV |
| [x] | 59_MED_29 | `src/components/dashboard/api-access-manager.tsx` (~745 lines) | API access manager too large — code examples should be separate constants file | 59_FRONTEND_DEV |
| [x] | 59_MED_30 | `uptime-display.tsx`, `dashboard-password.tsx` | Use raw fetch + useEffect instead of React Query — inconsistent with codebase | 59_FRONTEND_DEV |
| [x] | 59_MED_31 | `src/components/dashboard/logs-explorer.tsx` | Renders all filtered entries (500+) without virtualization — significant DOM churn | 59_FRONTEND_DEV |
| [x] | 59_MED_32 | `src/components/dashboard/mini-sparkline.tsx:39` | Gradient ID collision if two sparklines have same data | 59_FRONTEND_DEV |
| [x] | 59_MED_33 | `src/components/dashboard/overview-sparklines.tsx:29` | Query returns null instead of throwing on error — silently swallows failures | 59_FRONTEND_DEV |
| [x] | 59_MED_34 | `src/components/dashboard/openclaw-embed.tsx:25` | Auth fetch has no AbortController — state update on unmounted component possible | 59_FRONTEND_DEV |
| [x] | 59_MED_35 | `src/components/dashboard/logs-explorer.tsx:278-289` | Log filter badges are clickable but not keyboard-accessible (Badge onClick, not button) | 59_FRONTEND_DEV |
| [x] | 59_MED_36 | Dynamic content areas | Missing aria-live regions for polling content (VPS status, monitoring, notifications) | 59_FRONTEND_DEV |
| [x] | 59_MED_37 | `api-access-manager.tsx`, `openclaw-credentials-banner.tsx` | Key list items and credentials banner overflow on mobile — no flex-wrap | 59_FRONTEND_DEV |
| [x] | 59_MED_38 | `src/components/dashboard/vps-controls.tsx` | Chart tooltip may be clipped on mobile | 59_FRONTEND_DEV |
| [x] | 59_MED_39 | `agents/page.tsx:68-71` | Fragile assumption about Supabase join behavior — normalizes relation as array | 59_FRONTEND_DEV |
| [x] | 59_MED_40 | `vps-maintenance.tsx` | Scheduled restarts use mock data with misleading success toast | 59_FRONTEND_DEV |

### LOW

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | 59_LOW_01 | `src/app/dashboard/page.tsx:142` | Contact support link for no-subscription users is minor UX concern | 59_QA_TESTER (BUG-002) |
...
| [x] | 59_LOW_27 | `model-playground.tsx:226-239` | Uses raw textarea instead of shadcn Textarea | 59_FRONTEND_DEV |
| [x] | 59_LOW_28 | `api-playground.tsx:339` | Uses native checkbox instead of shadcn Checkbox | 59_FRONTEND_DEV |

### UX ISSUES (from 59_UX_REVIEWER)

| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | 59_UX_01 | No cancel/downgrade flow in Billing — users cannot leave | 59_UX_REVIEWER |
| [x] | 59_UX_02 | No self-service payment method update — must contact support | 59_UX_REVIEWER |
| [x] | 59_UX_03 | Chat page missing 4 built components (mobile sidebar, export, read receipts, sound toggle) | 59_UX_REVIEWER |
| [x] | 59_UX_04 | Security Score logic is broken — older accounts rated lower, meaningless metric | 59_UX_REVIEWER |
| [x] | 59_UX_05 | "Password Last Changed: Unknown" in security section | 59_UX_REVIEWER |
| [x] | 59_UX_06 | No file attachments in ticket creation (component exists but unused) | 59_UX_REVIEWER |
| [x] | 59_UX_07 | No conversation history browser in chat — /clear destroys everything | 59_UX_REVIEWER |
| [x] | 59_UX_08 | No alert thresholds on VPS metrics — no color change at 80%+ | 59_UX_REVIEWER |
| [x] | 59_UX_09 | Third sparkline card switches between Messages and Tickets — confusing | 59_UX_REVIEWER |
| [x] | 59_UX_10 | Avatar upload component exists but unused on Account page | 59_UX_REVIEWER |
| [x] | 59_UX_11 | 11 sidebar items without grouping — too many ungrouped | 59_UX_REVIEWER |
...
| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | 350_CRIT_01 | `dependencies/route.ts:99` | Cross-user dependency deletion — no user_id check; any authenticated user can delete any dependency | 350_FIX_VERIFICATION (FIX-03), 350_QA_POSTFIX, 350_BACKEND_POSTFIX |
| [x] | 350_CRIT_02 | `tasks/queue/route.ts:4-10` | Task queue route lacks guardMCRoute — no Ultra plan check, no rate limiting; any authenticated user can access | 350_FIX_VERIFICATION (FIX-04), 350_QA_POSTFIX, 350_BACKEND_POSTFIX |
...
| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | 350_UX_01 | No automation rules UI — backend built, frontend missing entirely; users cannot access the feature | 350_UX_POSTFIX |
| [x] | 350_UX_02 | No dependencies UI — API built, no visual way to add/view dependencies | 350_UX_POSTFIX |
...
| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | V1_CRIT_01 | All V1 routes | Auth boilerplate duplicated 25+ times (~20 lines each) — 2 DB queries per handler x 25 = 50 redundant calls | V1_API_BACKEND, V1_API_SECURITY, V1_API_QA_TESTER (BUG-026) |
| [x] | V1_CRIT_02 | No global limit | No global request body size limit — 100MB JSON parsed into memory before validation | V1_API_SECURITY (#6), V1_API_QA_TESTER (BUG-030) |
| [x] | V1_CRIT_03 | `chat/batch/route.ts:37` webhook_url | Batch webhook_url stored but never validated for SSRF — attacker can scan internal network | V1_API_SECURITY (#13) |

### HIGH

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | V1_HIGH_01 | `chat/route.ts:81-106` | Chat endpoint mixes 3 incompatible error formats — structured, manual, and flat string | V1_API_CONSUMER (BUG-01), V1_API_QA_TESTER (BUG-002), V1_API_BACKEND |
| [x] | V1_HIGH_02 | 5 endpoints | Missing plan check — batch/:id, files/:id GET/DELETE, predictions/:id, threads/:id/messages GET | V1_API_QA_TESTER (BUG-005), V1_API_BACKEND |
| [x] | V1_HIGH_03 | 11+ endpoints | No rate limiting — DELETE /agents/:id triggers SSH, could exhaust connections | V1_API_QA_TESTER (BUG-006), V1_API_SECURITY (#4), V1_API_BACKEND |
...
| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | LAND_HIGH_01 | All landing components | No prefers-reduced-motion support — infinite marquee, pulsing dots, auto-cycling all animate for vestibular disorder users | LANDING_FRONTEND |
| [x] | LAND_HIGH_02 | `Comparison.tsx`, `Pricing.tsx` | Comparison/pricing tables overflow horizontally on mobile with no scroll indicator | LANDING_FRONTEND |
| [x] | LAND_HIGH_03 | `Navbar.tsx` | Mobile menu toggle has no aria-label | LANDING_FRONTEND |
| [x] | LAND_HIGH_04 | `FAQ.tsx` | FAQ accordion not ARIA-compliant — no aria-expanded, aria-controls, or role | LANDING_FRONTEND |
| [x] | LAND_HIGH_05 | All landing components | No lazy loading — all 14 sections rendered immediately, no next/dynamic | LANDING_FRONTEND |
| [x] | LAND_HIGH_06 | Hero section | No product visual above the fold — text only, no screenshot/mockup/video | LANDING_UX_CONVERSION |
| [x] | LAND_HIGH_07 | Entire page | Zero social proof — no testimonials, customer logos, case studies | LANDING_UX_CONVERSION |

### MEDIUM

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | LAND_MED_01 | `ChatMockup` | Infinite typing loop at 20ms (50fps) with state updates every tick — expensive on mobile | LANDING_FRONTEND |
| [x] | LAND_MED_02 | `KanbanMockup` | Auto-move card reset causes visual "jump" when all cards exhausted | LANDING_FRONTEND |
| [x] | LAND_MED_03 | `Stats.tsx` | Stats component buttons lack aria-label or aria-current | LANDING_FRONTEND |
| [x] | LAND_MED_04 | `ProductTour.tsx` | Tab buttons implemented without role=tablist/tab/tabpanel or aria-selected | LANDING_FRONTEND |
| [x] | LAND_MED_05 | No skip-to-content link | Users must tab through entire navbar before reaching content | LANDING_FRONTEND |
| [x] | LAND_MED_06 | Focus indicators | Rely on browser defaults — may be invisible on dark backgrounds | LANDING_FRONTEND |
| [x] | LAND_MED_07 | `layout.tsx` | Missing og:image for social sharing — no preview card image | LANDING_FRONTEND, LANDING_BACKEND (M-2) |
| [x] | LAND_MED_08 | `layout.tsx` | Missing og:url and metadataBase | LANDING_BACKEND (M-1, M-3) |
| [x] | LAND_MED_09 | No robots.ts or sitemap.ts | Important for search engine discovery | LANDING_BACKEND (M-5) |
| [x] | LAND_MED_10 | `src/middleware.ts`, `next.config.ts` | No Content-Security-Policy headers | LANDING_SECURITY |
| [x] | LAND_MED_11 | `src/middleware.ts`, `next.config.ts` | No HSTS, X-Frame-Options, X-Content-Type-Options headers | LANDING_SECURITY |
| [x] | LAND_MED_12 | `HowItWorks.tsx` | Nested boxes indentation reaches ~96px on mobile — very narrow content | LANDING_FRONTEND |
| [x] | LAND_MED_13 | `/docs`, `/terms`, `/privacy` not in middleware matcher | No session refresh on public pages — session could expire during docs reading | LANDING_BACKEND (MW-1) |

### LOW

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | LAND_LOW_01 | `Navbar.tsx:87` | Logo href="#" — dead link, should be "/" | LANDING_FRONTEND, LANDING_QA_TESTER (BUG-001), LANDING_UX_CONVERSION |
| [x] | LAND_LOW_02 | `VPSHealthMockup` | Potential interval leak — inner interval cleanup not called on fast unmount | LANDING_FRONTEND |
| [x] | LAND_LOW_03 | `globals.css` | No scroll-behavior: smooth — anchor clicks snap-jump | LANDING_FRONTEND |
| [x] | LAND_LOW_04 | `CTA.tsx` | Giant text glow layer clone missing aria-hidden="true" | LANDING_FRONTEND |
| [x] | LAND_LOW_05 | `DashboardShowcase.tsx`, `Architecture.tsx` | Exist but not imported — dead components | LANDING_FRONTEND |
| [x] | LAND_LOW_06 | 8 mockup components | Not imported on landing page — may be unused across entire app | LANDING_FRONTEND |
| [x] | LAND_LOW_07 | `Features.tsx` | Injects <style> tag inside component body — anti-pattern | LANDING_FRONTEND |
| [x] | LAND_LOW_08 | `Footer.tsx:59` | GitHub link missing target="_blank" rel="noopener noreferrer" | LANDING_SECURITY |
| [x] | LAND_LOW_09 | `layout.tsx` | Missing og:siteName and canonical URL | LANDING_BACKEND (M-4, M-6) |

### UX/CONVERSION ISSUES

| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | LAND_UX_01 | Page section order suboptimal — Stats too early, WhyClawHQ after Pricing (should be before) | LANDING_UX_CONVERSION |
| [x] | LAND_UX_02 | No free trial, no demo, no freemium — $59/mo is high-friction first ask | LANDING_UX_CONVERSION |
| [x] | LAND_UX_03 | No CTA after Comparison section (most convincing section) | LANDING_UX_CONVERSION |
| [x] | LAND_UX_04 | Page too long — 13 sections, users may not reach Pricing | LANDING_UX_CONVERSION |
| [x] | LAND_UX_05 | Solutions dropdown links all go to #features — not granular | LANDING_UX_CONVERSION |

### TERMS/PRIVACY QA

| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | LAND_QA_01 | Terms feature descriptions conflict with landing page — Starter says "Single agent" vs 7 free agents; Mission Control listed as Pro instead of Ultra-only | LANDING_QA_TESTER (BUG-002) |
| [x] | LAND_QA_02 | Navbar logo casing inconsistency — "clawhq" vs "ClawHQ" across pages | LANDING_QA_TESTER |
...
| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | ADMIN_CRIT_01 | `admin/layout.tsx` | 2FA bypass — admin layout does NOT check MFA assurance level; 2FA is effectively decorative | ADMIN_QA_TESTER (A1/S1) |
| [x] | ADMIN_CRIT_02 | `admin/customers/[id]/route.ts` DELETE | Customer deletion does not deprovision VPS — orphaned servers remain running with valid SSH credentials | ADMIN_SECURITY |
| [x] | ADMIN_CRIT_03 | `admin/customers/[id]/full/route.ts` | GET /full returns SSH password, dashboard password, and data API token all in one response | ADMIN_SECURITY |

### HIGH

| Done | ID | File:Line | Description | Sources |
|------|----|-----------|-------------|---------|
| [x] | ADMIN_HIGH_01 | `admin/vps-password/route.ts` GET | Credential access not audit-logged | ADMIN_SECURITY |
...
| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | DB_CRIT_01 | `channel_credentials` has no user_id column and no RLS policies — data leak vector | DATABASE_ADMIN |
| [x] | DB_CRIT_02 | `ALL_NEW_MIGRATIONS.sql` is a duplicate concatenation — will cause conflicts on deploy | DATABASE_ADMIN |
| [x] | DB_CRIT_03 | Timestamp collision: `20260315400000` used by two files | DATABASE_ADMIN |
| [x] | DB_CRIT_04 | Timestamp collision: `20260315500000` used by two files | DATABASE_ADMIN |
| [x] | DB_CRIT_05 | `kb_feedback.chunk_id` and `document_id` have no FK constraints — orphan rows on delete | DATABASE_ADMIN |
| [x] | DB_CRIT_06 | SECURITY DEFINER functions accept p_user_id without validation — user could query another user's KB chunks | DATABASE_ADMIN |
| [x] | DB_CRIT_07 | `siem_configs` and `log_alert_rules` defined twice with conflicting schemas | DATABASE_ADMIN |
...
| Done | ID | Description | Sources |
|----|-------------|---------|
| [x] | PERF_P0_01 | Mission Control polling storm — 2+ API requests/second/user from overlapping refetchIntervals across components | PERFORMANCE_ENGINEER |
